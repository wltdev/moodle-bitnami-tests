...
/**
 * Strings for component 'enrol_authorize', language 'pt_br', version '4.4'.
 *
...
{$a->pending} transações para o curso "{$a->course}" expirarão a menos que você aceite o pagamento dentro de {$a->days} dias.

Este é um aviso, porque você não ativou a captura programada.
Isso significa que você deve aceitar ou recusar os pagamentos manualmente.

Para aceitar/recusar pagamentos pendentes, vá para:
{$a->url}

Para ativar a captura programada, o que significa que você não receberá mais e-mails de aviso, vá para:

{$a->enrolurl}';
$string['pendingordersemailteacher'] = 'Prezado professor,

{$a->pending} transações custaram {$a->currency} {$a->sumcost} para o curso "{$a->course}"
expirarão a menos que você aceite o pagamento dentro de {$a->days} dias.

Você deve aceitar ou recusar os pagamentos manualmente porque o administrador não ativou a captura programada.
