...
class quiz_attempt
{
    ...
    public function load_questions()
    {
        ...
        $this->sections = array_values($DB->get_records(
            'quiz_sections',
            ['quizid' => $this->get_quizid()],
            'firstslot'
        ));
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    if (
                        $this->slots[$slot]->displaynumber !== null && $this->slots[$slot]->displaynumber !== '' &&
    ...
    public function is_review_allowed()
    {
        ...
        $cm = $this->get_cm();
        if (
            $this->has_capability('moodle/site:accessallgroups') ||
    ...
    {
        return quiz_feedback_for_grade(
            $grade,
            $this->get_quiz(),
    ...
    public function get_display_options($reviewing)
    {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = quiz_get_review_options(
                    $this->get_quiz(),
                    $this->attempt,
                    $this->quizobj->get_context()
                );
    ...
    public function cannot_review_message($short = false)
    {
        return $this->quizobj->cannot_review_message(
            $this->get_attempt_state(),
            $short,
            $this->attempt->timefinish
        );
    }
    ...
    protected function render_question_helper(
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag(
                    'p',
    ...
    protected function make_blocked_question_placeholder($slot)
    {
        ...
        $placeholderqa = new question_attempt(
            $question,
            $this->quba->get_id(),
            null,
            $this->quba->get_question_max_mark($slot)
        );
    ...
    public function render_question_for_commenting($slot)
    {
        ...
        return $this->quba->render_question(
            $slot,
            $options,
    ...
    public function check_file_access(
        ...
        return $this->quba->check_file_access(
            $slot,
            $options,
            $component,
            $filearea,
    ...
    public function links_to_other_redos($slot, moodle_url $baseurl)
    {
        ...
        foreach ($qas as $qa) {
            if ($qa->get_slot() == $slot) {
                ...
                $links->links[$index] = new action_link(
                    $url,
                    $index,
                    new popup_action(
                        'click',
